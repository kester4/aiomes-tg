...
async def code_state(message: Message, state: FSMContext):
    ...
    await state.clear()
    auth, instance = data["inst"], data["playwright"]
...
async def start_auth(message: Message, state: FSMContext):
    ...
    try:
        ...
        if value == '2FA_NEEDED':
            ...
            await state.update_data(inst=auth, playwright=instance)
            await state.set_state(AuthState.enter_2fa)
            await asyncio.create_task(terminate_playwright(state, instance))
...
async def send_funcs(callback: CallbackQuery):
...
async def send_funcs(callback: CallbackQuery):
    ...
    if call_target == 'show':
        ...
        marks = await user.get_period_marks(user.class_level, int(call_target) - 1)
        text = f'<b>Аттестационный период {call_target}</b>\n\n{await parse_trimester(marks)}'

    await callback.message.edit_text(text, reply_markup=subject_marks_kb)
